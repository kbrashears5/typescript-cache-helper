...
export class CacheHelper implements ICacheHelper {
    ...
    constructor(cacheNames: string[],
        cacheExpirationSeconds: number = 300) {
        this.ObjectOperations = new ObjectOperations();

        if (this.ObjectOperations.IsNullOrEmpty(cacheNames)) throw new Error(`Value cannot be null or undefined: [cacheNames]`);
        if (cacheNames.length < 1) throw new Error(`Value cannot be null or undefined: [cacheNames]`);
     ...
     */
    public Add(cacheName: string,
        key: string,
        value: object): boolean {

        const action = `${CacheHelper.name}.${this.Add.name}`;

        if (this.ObjectOperations.IsNullOrWhitespace(cacheName)) throw new Error(`[${action}]-Value cannot be null or undefined: [cacheName]`);
        if (this.ObjectOperations.IsNullOrWhitespace(key)) throw new Error(`[${action}]-Value cannot be null or undefined: [key]`);
        if (this.ObjectOperations.IsNullOrEmpty(value)) throw new Error(`[${action}]-Value cannot be null or undefined: [value]`);
     ...
     * @param key {string} Cache item key
     */
    public Delete(cacheName: string,
        key: string): boolean {
     ...
     * @param cacheExpirationSeconds {number} Time to expiration for the cache in seconds
     */
    public Initialize(cacheNames: string[],
        cacheExpirationSeconds: number): void {

        const action = `${CacheHelper.name}.${this.Initialize.name}`;

        if (this.ObjectOperations.IsNullOrEmpty(cacheNames)) throw new Error(`[${action}]-Value cannot be null or undefined: [cacheNames]`);
     ...
     */
    public Replace(cacheName: string,
        key: string,
        value: object): void {
